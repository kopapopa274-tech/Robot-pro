...
export async function placeBlock(bot, blockType, x, y, z, placeOn='bottom', dontCheat=false) {
    ...
    // Guard against being asked to "place" something that isn't actually a placeable
    // block (e.g. a tool, weapon, or food item). Without this check, the function would
    // happily equip and swing whatever matching item it found in the inventory, log it
    // as successfully placed, and silently do nothing in-game.
    const placeable_names = ['water', 'lava']; // these use a bucket item but are real blocks
    if (!placeable_names.includes(blockType) && mc.getBlockId(blockType) == null) {
        log(bot, `${blockType} is not a placeable block. Cannot place it.`);
        return false;
    }
