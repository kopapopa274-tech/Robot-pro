...
import * as hybrid from './library/hybrid_skills.js';
...
const modes_list = [
    ...
    {
        name: 'fall_clutch',
        description: 'Detect an unsurvivable fall and negate it with a water/boat/soft-block MLG clutch. Interrupts all actions.',
        interrupts: ['all'],
        on: true,
        active: false,
        // deliberately does NOT go through execute()/runAction: a real clutch has to
        // happen within a tick or two, and runAction's action-queue plumbing is not
        // built for that kind of latency. This mode manages bot.modes.active itself.
        update: async function (agent) {
            const bot = agent.bot;
            if (!hybrid.needsFallClutch(bot)) return;
            if (this.active) return;
            this.active = true;
            try {
                const clutched = await hybrid.attemptFallClutch(bot);
                if (clutched) agent.bot.modes.behavior_log += 'Performed an MLG fall clutch.\n';
            } catch (err) {
                console.log('fall_clutch error:', err);
            } finally {
                this.active = false;
            }
        }
    ...
    {
        name: 'unstuck',
        description: 'Multi-tier escalation to get unstuck: dig out surroundings, then pillar up, then move away. Interrupts some actions.',
        interrupts: ['all'],
        on: true,
        active: false,
        prev_location: null,
        ...
        max_stuck_time: 20,
        prev_dig_block: null,
        stage: 0, // 0 = not escalated yet, 1 = tried digging, 2 = tried pillaring, 3 = fallback move-away
        update: async function (agent) {
            if (agent.isIdle()) {
            ...
            if (this.stuck_time > max_stuck_time) {
                this.stuck_time = 0;
                const stageToRun = Math.min(this.stage + 1, 3);
                execute(this, agent, async () => {
                    const crashTimeout = setTimeout(() => { agent.cleanKill("Got stuck and couldn't get unstuck") }, 15000);
                    let resolved = false;
                    if (stageToRun === 1) {
                        say(agent, 'I\'m stuck! Digging myself out...');
                        resolved = await hybrid.digOutStuck(bot).catch(() => false);
                    }
                    else if (stageToRun === 2) {
                        say(agent, 'Still stuck. Pillaring up...');
                        resolved = await hybrid.pillarUp(bot, 4).catch(() => false);
                    }
                    else {
                        say(agent, 'Still stuck. Moving away...');
                        resolved = await skills.moveAway(bot, 5).catch(() => false);
                    }
                    clearTimeout(crashTimeout);
                    // escalate next time if this stage didn't actually change anything,
                    // otherwise stay at this stage in case the same spot traps us again
                    this.stage = resolved ? stageToRun : Math.min(stageToRun + 1, 3);
                    say(agent, resolved ? 'I\'m free.' : 'That didn\'t work, trying something else next.');
                });
            }
    ...
    {
        name: 'self_defense',
        description: 'Attack nearby enemies, using a trap first if the fight is going badly. Interrupts all actions.',
        interrupts: ['all'],
        on: true,
        active: false,
        update: async function (agent) {
            ...
            if (enemy && await world.isClearPath(agent.bot, enemy)) {
                ...
                execute(this, agent, async () => {
                    await hybrid.tacticalDefendSelf(agent.bot, 8);
                });
            }
        }
    },
    ...
    },
    {
        ...
        description: 'When idle, automatically craft and equip the next tier of tools/weapons/armor if materials are already on hand. Never goes out to gather.',
        interrupts: [],
        on: true,
        active: false,
        cooldown: 30, // seconds between attempts, this is a background chore not a reflex
        last_attempt: 0,
        update: async function (agent) {
            ...
            execute(this, agent, async () => {
                const upgraded = await hybrid.progressAllGear(agent.bot);
                if (upgraded) say(agent, 'Upgraded my gear.');
            });
        }
    },
    {
        name: 'auto_cook',
        description: 'When idle and hunger is low, cook any raw food on hand if a furnace and fuel are available.',
        interrupts: [],
        on: true,
        active: false,
        hunger_threshold: 15,
        ...
        update: async function (agent) {
            const bot = agent.bot;
            if (!agent.isIdle()) return;
            if (bot.food >= this.hunger_threshold) return;
            if (Date.now() - this.last_attempt < this.cooldown * 1000) return;
            this.last_attempt = Date.now();
            execute(this, agent, async () => {
                const cooked = await hybrid.autoCookRawFood(bot);
                if (cooked) say(agent, 'Cooked some food.');
            });
        }
    },
    {
        name: 'auto_shelter',
        description: 'When idle at night with no bed nearby, build a basic shelter to survive the night and set a spawn point. Runs at most once per session.',
        interrupts: [],
        on: true,
        active: false,
        attempted: false,
        update: async function (agent) {
            ...
            if (!agent.isIdle()) return;
            if (!bot.time || bot.time.timeOfDay < 12500 || bot.time.timeOfDay > 23000) return; // only near/at night
            const nearbyBed = world.getNearestBlock(bot, 'bed', 30) || world.getNearestBlock(bot, 'white_bed', 30);
            if (nearbyBed) { this.attempted = true; return; } // already has shelter/spawn nearby
            this.attempted = true; // one attempt per session, whether or not it succeeds
            say(agent, 'Nightfall - building a shelter.');
            execute(this, agent, async () => {
                await hybrid.buildShelter(bot);
                const bed = world.getNearestBlock(bot, 'bed', 10) ||
                    bot.inventory.items().find(i => i.name.endsWith('_bed')) && world.getNearestBlock(bot, 'bed', 30);
                if (bed) {
                    try { await skills.goToBed(bot); } catch (err) { /* not fatal if sleeping fails */ }
                }
            });
        }
    },
...
async function execute(mode, agent, func, timeout=-1) {
    ...
    if (should_reprompt) {
        ...
        Your behavior log: ${logs}\nRespond accordingly.`)
            .catch(err => console.error('Error handling mode interruption reprompt:', err));
    }
}
